...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "QUERY FORMAT — FTS5 AND search:\n"
        "This is a keyword AND search engine. Every word in your query must appear in a message for it to match. "
        "Do NOT write natural language sentences or descriptions as queries — they will almost always return zero results.\n\n"
        "Rules:\n"
        "- Use 1-3 distinctive keywords. Single keywords work best.\n"
        "- Join with OR for broad recall: elevenlabs OR baseten OR funding\n"
        "- Phrase match with quotes: \"docker networking\"\n"
        "- Boolean: python NOT java, prefix: deploy*\n"
        "- If a query returns nothing, try a DIFFERENT single keyword — do NOT add more words to the failed query.\n\n"
        "Examples:\n"
        "- ✅ Treue → finds all sessions mentioning Treue\n"
        "- ✅ Aave → finds all sessions mentioning Aave\n"
        "- ✅ Yasin OR Asmaiyt → broad recall across both names\n"
        "- ❌ Treue clubbing night → ANDs all three, almost always zero results\n"
        "- ❌ Aave WETH frozen position → ANDs all four, misses most sessions about Aave\n"
        "- ❌ session about the robot calibration → natural language, ANDs every common word"
    ),
    "parameters": {
        "type": "object",
        "properties": {
