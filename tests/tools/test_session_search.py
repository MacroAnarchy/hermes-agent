...
class TestSessionSearch:
    ...
    def test_current_root_session_includes_delegation_child(self):
        ...
        assert result["success"] is True
        # Child/delegation sessions can contain content not present in the root
        # transcript, so searching from the root should still surface matching
        # child content resolved back to the root session.
        assert result["count"] == 1
        assert result["sessions_searched"] == 1
